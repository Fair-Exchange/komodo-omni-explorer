...
class InterestCalc extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      interestSAFEFiatPrice: this.props.Main.fiatRates && this.props.Main.fiatRates.USD || 0,
  ...
  componentWillReceiveProps(props) {
    if (props &&
      ...
      this.setState({
        interestSAFEFiatPrice: props.Main.fiatRates.USD,
      });
    }
  }
  ...
  getInterestData(frequency, _total) {
    ...
    for (let i = 0; i < Math.floor(frequency); i++) {
      ...
      _items.push(<tr key={ `interest-calc-months-yearly-${i}` }>
        ...
        <td>${ Number(_totalAmounts[i] * this.state.interestSAFEFiatPrice).toFixed(3) }</td>
      </tr>
      );
    }
      ...
      case 'year':
        ...
        _items.push(
          <tr key={ `interest-calc-days-year` }>
            ...
            <td>${ Number(_total * this.state.interestSAFEFiatPrice).toFixed(3) }</td>
          </tr>
        );
        break;
      case 'months':
      ...
      <div>
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <p>Expenses not included in calculation: <strong>{ Number(_fees.toFixed(4)) } SAFE</strong> in transaction fees, <strong>{ _hoursGap } hour(s)</strong> gap period when no interest is accrued.</p>
        ...
        <div
          ...
          <div className="col-md-3 col-sm-3">
            <input
              onChange={ (event) => this.updateInput(event) }
              type="text"
              name="interestSAFEFiatPrice"
              disabled={ this.state.toggleInterestFiatAutoRate }
              value={ this.state.interestSAFEFiatPrice }
              placeholder="SAFE / USD rate"
              className="form-control" />
          </div>
          <div className="col-md-3 col-sm-3">
