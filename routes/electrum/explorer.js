...
const safecoinParams = require('../../config').safecoinParams;
...
module.exports = (shepherd) => {
  ...
  shepherd.get('/safe/interest', (req, res, next) => {
    const randomServer = _electrumServers.safe.serverList[getRandomIntInclusive(0, 1)].split(':');
            ...
            if (_utxo &&
              ...
              Promise.all(_utxo.map((_utxoItem, index) => {
                return new Promise((resolve, reject) => {
                  ...
                  .then((_rawtxJSON) => {
                    // decode tx
                    const decodedTx = txDecoder(_rawtxJSON, safecoinParams);
  ...
  shepherd.get('/safe/listunspent', (req, res, next) => {
    const network = 'safecoin';
    const randomServer = _electrumServers.safe.serverList[getRandomIntInclusive(0, 1)].split(':');
    const ecl = new shepherd.electrumJSCore(randomServer[1], randomServer[0], 'tcp');
