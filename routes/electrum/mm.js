...
for (let key in electrumCoins) {
  safePairs.push(`SAFE/${key}`);
  safePairs.push(`${key}/SAFE`);
}

console.log(`total orderbook pairs ${safePairs.length}`);
...
module.exports = (shepherd) => {
    ...
    function _getRates() {
      ...
      request(options, (error, response, body) => {
        if (response &&
          ...
          console.log(`unable to retrieve SAFE/BTC,USD rate`);
        }
      });
    }
  ...
  // get safe rates
  shepherd.get('/rates/safe', (req, res, next) => {
  ...
  shepherd.get('/mm/orderbook/start', (req, res, next) => {
    if (!shepherd.mm.orderbookLaunched) {
      ...
      const runOrdersUpdate = () => {
        ...
        async.eachOfSeries(safePairs, (value, key, callback) => {
          ...
          request(options, (error, response, body) => {
            if (response &&
              ...
              if (_callsCompleted === safePairs.length) {
                console.log('done');
